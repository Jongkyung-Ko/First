(function () {
  ...
  const BGM_PATTERNS = {
    cave: {
      tempo: 92,
      bass: [110, 0, 110, 98, 87, 0, 98, 110],
      lead: [440, 0, 523, 0, 392, 0, 349, 0],
      wave: "triangle"
    },
    flappy: {
      tempo: 128,
      bass: [196, 196, 220, 220, 247, 247, 220, 220],
      lead: [784, 880, 988, 880, 784, 659, 784, 880],
      wave: "square"
    },
    runner: {
      tempo: 148,
      bass: [130, 0, 146, 0, 164, 0, 146, 0],
      lead: [523, 587, 659, 587, 523, 494, 523, 587],
      wave: "square"
    },
    arcade: {
      tempo: 120,
      bass: [164, 164, 196, 196, 220, 220, 196, 164],
      lead: [659, 0, 784, 659, 988, 0, 784, 659],
      wave: "square"
    }
  };
  ...
  function playSynthBgm(track) {
    ...
    function playNote(freq, wave, len) {
      if (!freq) return;
      const osc = ctx.createOscillator();
      const g = ctx.createGain();
      osc.type = wave;
      ...
      if (oscillators.length > 24) oscillators.splice(0, 8);
    }

    const timer = setInterval(() => {
      if (muted) return;
      const i = step % pat.bass.length;
      playNote(pat.bass[i], "triangle", beatMs / 1000 * 0.9);
      playNote(pat.lead[i], pat.wave, beatMs / 1000 * 0.85);
      step++;
    }, beatMs);
