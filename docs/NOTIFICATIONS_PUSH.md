...
- **한국장**: GitHub Actions `record_kr`(06:50 KST) 후 `/api/notifications/cron/digest?region=kr`(07:08 KST)
- **미국장**: `record_us` 후 `region=us`
- **앱 테스트**: 추천공식 → 알림 카드 → 「지금 테스트 발송」
- **종목 필터**: 감성뉴스는 **당일** `trade_date`(06:50 record 직후) · 전략은 **전일** `signalDate` (18:00 스냅샷)
